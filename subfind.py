...
Please do not modify the code without permission or use it for any malicious use
-afro_coder
...
class podnapsi():
	...
	def get_sublist(self,subname,episode,season,*args,**kwargs):
		website="https://www.podnapisi.net/subtitles/search/?movie_type=&episodes="+episode+"&year=&keywords="+subname+"&seasons="+season+"&type="
			...
			podsoup=BeautifulSoup(podpage.content,"lxml")
			
			finalsoup=podsoup.find_all('span',{'class':'release'}) #finds titles
			...
			for number,title in enumerate(finalsoup,start=1):
				...
				#print(number,span['title'],title['data-href'].split('/')[2])
...
def main():
	...
	try:
		...
		ne=podnapsi()
		subparse=argparse.ArgumentParser(prog="subfind",usage="./main 'subtitle-name' season episode-number")
